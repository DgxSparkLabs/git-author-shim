...
- **Gap 2: strong "irreducible offline" claim refuted; ceiling clarified.** Real `omp`+`claude` were driven to spawn `git` and hit the shim; only *unauthenticated* startup stays untested. That drive is shell/tool-routed, so the no-shell contract stays Gap 1's; the stub remains the always-on hosted oracle, while the real-binary drive is now a **committed self-hosted/manual `realism` gate** (see "Real-binary CI test — implemented" below), not a hand-run.
...
**Tier 2 — self-hosted/manual realism gate: drive the real binaries. IMPLEMENTED (see below).**
- Non-interactive driver adopted for the committed gate: **`omp -p` — proven** (`omp -p --no-session --no-pty --auto-approve --cwd <repo> "<git instruction>"` runs the model, spawns `git`, exits in ~18–21s; model-driven, so it needs auth+network+a little quota). Not adopted: (a) `omp --mode rpc` + `{"type":"bash",…}` — **unverified** (a live attempt got no bash-result frame back, only command-catalog re-emission), a possible zero-quota upgrade only if the frame shape is confirmed; (b) `claude -p --allowedTools "Bash(git *)"` / `claude --bg --exec` — untested, addable as a second driver once verified.
- The interactive REPL `!` escape (omp `!git …`, claude `! …`) is now only a **manual fallback**: it proved the chain by hand but is fragile (the `!` must sit at column 0, or the line routes to the model) and is superseded by `omp -p` for automation. Windows PTY: `pexpect` does not work natively and zellij is Unix/WSL-only. The gate is never required for merge and never given secrets in Actions.
...
## Real-binary CI test — implemented (2026-09-09)

Tier 2 is no longer only a design. A committed, opt-in test now launches the **real** binary:
