...
- **Shell Wrapper Tools (direnv, asdf, mise)**: Existing environment managers rely on static declarations in `.envrc` or config files to switch tool behavior per directory or context without modifying global system state — [source: direnv.net / open-source prior art]
...
- **Accidental False-Positive Risk with Wildcards**: If wildcard matching rules are too permissive, a human user might accidentally export a variable (e.g., `AIDER_THEME=dark` or `CURSOR_FONT=12`) that triggers agent mode unintentionally — [source: security analysis of environment variable matching]
