...
- [x] T002 [P] Create initial package skeleton with `src/git_author_shim/__init__.py`
...
- [x] T007 Implement core data structures and redaction formatting in `src/git_author_shim/data_models.py` (gates on T006 passing)
...
- [x] T009 Implement TOML configuration loader and validator in `src/git_author_shim/config.py` (gates on T008 passing)
...
- [x] T011 Implement real Git binary resolution and recursion sentinel handling in `src/git_author_shim/real_git_discovery.py` (gates on T010 passing)
...
- [x] T013 Implement agent mode and vendor marker detection in `src/git_author_shim/agent_detection.py` (gates on T012 passing)
...
- [x] T015 [US1] Implement canonical URL normalization and wildcard matcher in `src/git_author_shim/repo_identity_matcher.py` (gates on T014 passing)
...
- [x] T017 [US1] Implement SSH command generator with forward-slash formatting and host validation in `src/git_author_shim/credential_injector.py` (gates on T016 passing)
...
- [x] T019 [US1] Implement CLI execution engine in `src/git_author_shim/cli.py` and `src/git_author_shim/__main__.py` with lazy imports, Windows `subprocess.run` exit code propagation, signal handlers, and stdio forwarding (gates on T018 passing)
...
- [x] T021 [US1] Wire end-to-end SSH bot commit and push execution in `src/git_author_shim/cli.py` (gates on T020 passing)
...
- [x] T023 [US2] Implement human-mode fast short-circuit in `src/git_author_shim/cli.py` (gates on T022 passing)
...
- [x] T025 [US3] Implement dynamic HTTPS credential helper generation with multi-source token resolution in `src/git_author_shim/credential_injector.py` (gates on T024 passing)
...
- [x] T027 [US3] Wire HTTPS credential helper injection into `src/git_author_shim/cli.py` (gates on T026 passing)
...
- [x] T029 [US4] Implement command taxonomy classifier in `src/git_author_shim/commit_authorship_classifier.py` (gates on T028 passing)
...
- [x] T031 [US4] Implement fail-closed write refusal and human override in `src/git_author_shim/cli.py` (gates on T030 passing)
...
- [x] T033 [US6] Enforce `__GIT_SHIM_CONTINUATION` propagation and child invocation security in `src/git_author_shim/real_git_discovery.py` and `src/git_author_shim/cli.py` (gates on T032 passing)
...
- [x] T035 [US7] Implement batched sequencer state inspection in `src/git_author_shim/commit_authorship_classifier.py` (gates on T034 passing)
...
- [x] T037 [US7] Implement author-preservation vs author-overwrite injection logic in `src/git_author_shim/cli.py` (gates on T036 passing)
...
- [x] T039 [US8] Implement SHA-256 content hashing and trusted registry persistence in `src/git_author_shim/repo_config_trust.py` (gates on T038 passing)
...
- [x] T041 [US8] Integrate trust verification into `src/git_author_shim/config.py` and implement `trust`/`untrust` subcommands in `src/git_author_shim/cli.py` (gates on T040 passing)
...
- [x] T043 [US9] Implement pre-flight credential existence checks and effective destination host validation in `src/git_author_shim/credential_injector.py` and `src/git_author_shim/cli.py` (gates on T042 passing)
...
- [x] T045 [US5] Implement `explain` (text and JSON) and `list-identities` formatting with strict secret redaction in `src/git_author_shim/cli.py` (gates on T044 passing)
...
- [x] T047 [US10] Implement specificity scoring and tie-breaker refusal in `src/git_author_shim/repo_identity_matcher.py` (gates on T046 passing)
...
- [x] T049 [US11] Implement tagger classification and environment injection in `src/git_author_shim/commit_authorship_classifier.py` and `src/git_author_shim/cli.py` (gates on T048 passing)
...
- [x] T051 [US12] Classify `stash` and `notes` as originating writes in `src/git_author_shim/commit_authorship_classifier.py` (gates on T050 passing)
